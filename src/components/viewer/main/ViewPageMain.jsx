...
export default function ViewPageMain() {
  ...
  const fetchArticles = async (page = 0, resultsPerPage = 5, searchTerm = '', lineOfResearchId = '', year = '', onGoing = '') => {
    setIsFetching(true);
    try {
      resultsPerPage = resultsPerPage ?? 5;
      const data = await projectsService.list(page, resultsPerPage, 'title,asc', searchTerm, lineOfResearchId, year, onGoing);
